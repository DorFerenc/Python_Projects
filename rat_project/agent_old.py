...
def get_memory_info():
    """
    Reads /proc/meminfo and returns summarized memory statistics.
    """
    try:
        with open("/proc/meminfo", "r") as f:
            lines = f.readlines()
            important = lines[:10]  # First 10 lines are enough for overview
...
def get_top_memory_processes():
    """
    Uses 'ps' to return top 5 processes by memory usage.
    """
    try:
        cmd = "ps aux --sort=-%mem | head -n 6"  # Header + 5 processes
